...
export function getEnvBaseUploadUrl() {
  // 请求基准地址
  let baseUploadUrl = import.meta.env.VITE_SERVER_BASEURL + import.meta.env.VITE_UPLOAD_BASEURL;
 ...
 * @returns
 */
export function uniNavigateTo(url: string, options?: any) {
 ...
 * @returns
 */
export function formValidate(formEl, prop?: string | string[]): Promise<boolean> {
  return new Promise((resolve, reject) => {
    formEl
      .validate(prop)
